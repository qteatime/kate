...
have a :ref:`cartridge build configuration <cartridge build configuration>` and run the a packaging application
to :ref:`package your game <package your game>`. You can find an example configuration using the
...
so you need to use the ``root`` option to change the directory the packaging
application uses for the game assets. The minimal configuration for a Ren'Py game
looks like this:
