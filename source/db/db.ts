...
export class Database {
  ...
  async transaction<A>(tables: TableSchema<string, string, any>[], mode: IDBTransactionMode, fn: (_: Transaction) => A): Promise<A> {
...
export class Transaction {
  ...
  get_table<T extends TableSchema<string, string, any>>(table: T) {
    return new Table<T['key']['path'], T['__schema']>(this.trans.objectStore(table.name));
  }
}
