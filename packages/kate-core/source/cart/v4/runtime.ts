...
export function parse_runtime(cart: Cart_v4.Cartridge): Runtime {
  ...
  switch (platform["@variant"]) {
    case Cart_v4.Runtime.$Tags.Web_archive: {
      return {
        type: "web-archive",
...
function bridge(x: Cart_v4.Bridge): Bridge {
  switch (x["@variant"]) {
    ...
    case Cart_v4.Bridge.$Tags.Network_proxy: {
      return { type: "network-proxy" };
    }
    case Cart_v4.Bridge.$Tags.Preserve_WebGL_render: {
      return { type: "preserve-render" };
    }
    case Cart_v4.Bridge.$Tags.Capture_canvas: {
      return { type: "capture-canvas", selector: str(x.selector, 255) };
    }
    case Cart_v4.Bridge.$Tags.Pointer_input_proxy: {
      return {
    ...
    case Cart_v4.Bridge.$Tags.IndexedDB_proxy: {
...
function virtual_key(key: Cart_v4.Virtual_key): KateButton {
  switch (key["@variant"]) {
    case Cart_v4.Virtual_key.$Tags.Capture:
      return "capture";
    case Cart_v4.Virtual_key.$Tags.Menu:
      return "menu";
    case Cart_v4.Virtual_key.$Tags.Up:
      return "up";
    case Cart_v4.Virtual_key.$Tags.Right:
      return "right";
    case Cart_v4.Virtual_key.$Tags.Down:
      return "down";
    case Cart_v4.Virtual_key.$Tags.Left:
      return "left";
    case Cart_v4.Virtual_key.$Tags.O:
      return "o";
    case Cart_v4.Virtual_key.$Tags.X:
      return "x";
    case Cart_v4.Virtual_key.$Tags.L_trigger:
      return "ltrigger";
    case Cart_v4.Virtual_key.$Tags.R_trigger:
      return "rtrigger";
    default:
...
function keyboard_key(key: Cart_v4.Keyboard_key): KeyboardKey {
